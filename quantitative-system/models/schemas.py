...
class PositionUpdate(BaseSchema):
    """更新持仓Schema"""
    name: Optional[str] = Field(None, max_length=100, description="股票名称")
    quantity: Optional[int] = Field(None, gt=0, description="持仓数量")
    avg_cost: Optional[Decimal] = Field(None, gt=0, description="平均成本")
    current_price: Optional[Decimal] = Field(None, gt=0, description="当前价格")
    status: Optional[PositionStatus] = Field(None, description="状态")
    close_date: Optional[date] = Field(None, description="平仓日期")
    notes: Optional[str] = Field(None, max_length=500, description="备注")
...
class PositionResponse(PositionBase):
    ...
    unrealized_pnl: Optional[Decimal] = Field(None, description="浮动盈亏")
...
class BacktestUpdate(BaseSchema):
    """更新回测Schema"""
    name: Optional[str] = Field(None, max_length=200, description="回测名称")
    status: Optional[BacktestStatus] = Field(None, description="状态")
    final_value: Optional[Decimal] = Field(None, description="最终价值")
    ...
    avg_trade_return: Optional[Decimal] = Field(None, description="平均交易收益")
...
class TradingPlanBase(BaseSchema):
    ...
    max_drawdown_limit: Optional[Decimal] = Field(None, description="最大回撤限制")
    position_limit: Optional[Decimal] = Field(None, description="仓位限制")
    notes: Optional[str] = Field(None, max_length=1000, description="备注")


class TradingPlanCreate(TradingPlanBase):
    """创建交易方案Schema"""
    recommendations: Optional[Dict[str, Any]] = Field(None, description="推荐操作")
...
class TradingPlanUpdate(BaseSchema):
    """更新交易方案Schema"""
    title: Optional[str] = Field(None, max_length=200, description="方案标题")
    content: Optional[str] = Field(None, description="方案内容")
    status: Optional[PlanStatus] = Field(None, description="状态")
    execution_rate: Optional[Decimal] = Field(None, description="执行率")
    actual_return: Optional[Decimal] = Field(None, description="实际收益率")
    notes: Optional[str] = Field(None, max_length=1000, description="备注")
...
class PlanGenerationRequest(BaseSchema):
    """方案生成请求Schema"""
    plan_date: Optional[date] = Field(None, description="方案日期，默认为今天")
...
class TaskCreate(TaskBase):
    """创建任务Schema"""
    created_by: Optional[str] = Field(None, max_length=50, description="创建者")
