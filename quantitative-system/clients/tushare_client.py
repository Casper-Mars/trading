        ...
        for _, income_row in income_df.iterrows():
            ...
            data = FinancialData(
                ...
                total_assets=self._safe_float(
                    balance_row.iloc[0].get("total_assets")
                    if not balance_row.empty
                    else None
                ),
                total_liab=self._safe_float(
                    balance_row.iloc[0].get("total_liab")
                    if not balance_row.empty
                    else None
                ),
                total_hldr_eqy_exc_min_int=self._safe_float(
                    balance_row.iloc[0].get("total_hldr_eqy_exc_min_int")
                    if not balance_row.empty
                    else None
                ),
                # 现金流量表数据
                n_cashflow_act=self._safe_float(
                    cashflow_row.iloc[0].get("n_cashflow_act")
