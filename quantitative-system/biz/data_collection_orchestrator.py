...
class DataCollectionOrchestrator(BaseOrchestrator):
    ...
    async def _pre_check(self, request: DataCollectionRequest, context: OrchestrationContext) -> None:
        """前置检查
        ...
        """
        logger.info(f"开始前置检查, 任务类型: {request.task_type}, request_id: {context.request_id}")

        # 验证请求参数
        self._validate_request(request, context)

        # 检查任务类型支持
        supported_types = [TaskType.STOCK_BASIC_COLLECTION, TaskType.DAILY_DATA_COLLECTION,
        ...
        if running_tasks and not request.force_update:
            raise OrchestrationError(f"已有正在执行的{request.task_type}任务, 请等待完成或使用force_update参数")
    ...
    async def _call_services(self, request: DataCollectionRequest, context: OrchestrationContext) -> dict[str, Any]:
        """调用服务
        ...
        """
        logger.info(f"开始服务调用, 任务类型: {request.task_type}, request_id: {context.request_id}")
        ...
        try:
            ...
            collection_result = await self._safe_service_call(
                'data_collection',
                lambda: self._execute_data_collection(request, task_result['task_id'], context),
                context
            )
            ...
            if request.quality_check and collection_result.get('collected_data'):
                quality_result = await self._safe_service_call(
                    'quality_check',
                    lambda: self._perform_quality_check(collection_result['collected_data'], request.task_type, context),
                    context
                )
            ...
            await self._safe_service_call(
                'task_update',
                lambda: self._update_task_status(task_result['task_id'], results, context),
                context
            )
            ...
            if 'task' in results and 'task_id' in results['task']:
                await self._mark_task_failed(results['task']['task_id'], str(e), context)
            raise

    async def _aggregate_results(self, service_results: dict[str, Any], context: OrchestrationContext) -> DataCollectionResponse:
        """聚合结果
        ...
        try:
            ...
            # 计算执行时间
            start_time = self._get_context_data('pre_check_time', context, datetime.now())
            ...
            response = DataCollectionResponse(
                ...
                valid_records=quality_result.get('valid_records', collection_result.get('processed_records', 0)),
            ...
            logger.info(f"结果聚合完成, 任务ID: {response.task_id}, 质量分数: {response.quality_score}, request_id: {context.request_id}")
            return response
    ...
    async def _create_collection_task(self, request: DataCollectionRequest, context: OrchestrationContext) -> dict[str, Any]:
        """创建采集任务
        ...
        Returns:
            任务创建结果
        """
        logger.info(f"创建采集任务, 类型: {request.task_type}, request_id: {context.request_id}")

        task_name = f"{request.task_type.value}_采集_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        self._add_rollback_action(
            'delete_data',
            {'task_id': task.id, 'table': 'tasks'},
            context
        )

        logger.info(f"任务创建成功, 任务ID: {task.id}, request_id: {context.request_id}")
    ...
    async def _execute_data_collection(self, request: DataCollectionRequest, task_id: int, context: OrchestrationContext) -> dict[str, Any]:
        """执行数据采集
        ...
        Returns:
            数据采集结果
        """
        logger.info(f"开始数据采集, 任务类型: {request.task_type}, 任务ID: {task_id}, request_id: {context.request_id}")
        ...
        try:
            if request.task_type == TaskType.STOCK_BASIC_COLLECTION:
                # 采集股票基础信息
                processed_records = await self.collection_service.collect_stock_basic_info(
                    force_update=request.force_update
                )
                # 获取采集的数据用于质量检查
                if request.quality_check:
                    collected_data = await self.stock_repo.get_recent_stock_basic_info(limit=processed_records)
                ...
                # 获取采集的数据用于质量检查
                if request.quality_check:
                    collected_data = await self.stock_repo.get_daily_data_by_date(target_date, limit=processed_records)

            elif request.task_type == TaskType.FINANCIAL_DATA_COLLECTION:
                # 采集财务数据
                processed_records = await self.collection_service.collect_financial_data(
                    symbols=request.symbols,
                    force_update=request.force_update
                )
                # 获取采集的数据用于质量检查
                if request.quality_check:
                    collected_data = await self.stock_repo.get_recent_financial_data(limit=processed_records)

            total_records = processed_records

            # 更新任务进度
            await self.task_repo.update_task_progress(task_id, processed_records, total_records)

            logger.info(f"数据采集完成, 处理记录数: {processed_records}, 任务ID: {task_id}, request_id: {context.request_id}")
            ...
            logger.error(f"{error_msg}, 任务ID: {task_id}, request_id: {context.request_id}")
            raise DataCollectionError(error_msg) from e

    async def _perform_quality_check(self, collected_data: list[dict], data_type: TaskType, context: OrchestrationContext) -> dict[str, Any]:
        """执行数据质量检查
        ...
        Returns:
            质量检查结果
        """
        logger.info(f"开始数据质量检查, 数据类型: {data_type}, 数据量: {len(collected_data)}, request_id: {context.request_id}")
        ...
        try:
            ...
            # 执行质量检查
            quality_report = self.quality_service.get_data_quality_report(collected_data, quality_data_type)

            logger.info(f"数据质量检查完成, 质量分数: {quality_report.get('quality_score', 0)}, request_id: {context.request_id}")
    ...
    async def _update_task_status(self, task_id: int, results: dict[str, Any], context: OrchestrationContext) -> None:
        """更新任务状态
        ...
        Args:
            ...
            context: 编排上下文
        """
        logger.info(f"更新任务状态, 任务ID: {task_id}, request_id: {context.request_id}")
        ...
        try:
            ...
            result_data = {
                ...
                'collection_time': collection_result.get('collection_time', datetime.now()).isoformat(),
            ...
            logger.info(f"任务状态更新完成, 任务ID: {task_id}, request_id: {context.request_id}")
    ...
    async def _mark_task_failed(self, task_id: int, error_message: str, context: OrchestrationContext) -> None:
        """标记任务失败
        ...
        try:
            ...
            logger.info(f"任务已标记为失败, 任务ID: {task_id}, 错误: {error_message}, request_id: {context.request_id}")
        except Exception as e:
            logger.error(f"标记任务失败时出错, 任务ID: {task_id}, 错误: {e!s}, request_id: {context.request_id}")
