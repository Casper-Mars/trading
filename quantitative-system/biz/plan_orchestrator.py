...
class PlanOrchestrator(BaseOrchestrator):
    ...
    def __init__(self, data_service: DataService, backtest_service: BacktestService, ai_service: AIService, plan_service: PlanService):
        """初始化方案生成编排器
    ...
    async def _pre_check(self, request: PlanGenerationRequest, context: OrchestrationContext) -> None:
        """前置检查
        ...
        """
        logger.info(f"Starting pre-check for plan generation, request_id: {context.request_id}")
    ...
    async def _call_services(self, request: PlanGenerationRequest, context: OrchestrationContext) -> dict[str, Any]:
        """调用服务
        ...
        """
        logger.info(f"Starting service calls for plan generation, request_id: {context.request_id}")
        ...
        try:
            ...
            data_tasks.append(self._safe_service_call(
                "data_service",
            ...
            ai_analysis = await self._safe_service_call(
                "ai_service",
                lambda: self.ai_service.analyze(analysis_request),
                context
            )
            ...
            logger.info(f"All service calls completed successfully, request_id: {context.request_id}")
    ...
    async def _aggregate_results(self, service_results: dict[str, Any], context: OrchestrationContext) -> PlanGenerationResponse:
        """聚合结果
        ...
        try:
            ...
            data_summary = {
                ...
                'data_points': len(stock_data.get('price_data', [])) if stock_data else 0,
                'market_data_available': market_data is not None,
                'backtest_completed': backtest_result is not None,
                'analysis_type': self._get_context_data('analysis_type', context, 'unknown')
            }
            ...
            execution_steps = self._generate_execution_steps(
                ai_analysis,
                self._get_context_data('symbols', context, []),
                context
            )
    ...
    def _generate_execution_steps(self, ai_analysis: dict[str, Any], symbols: list[str], context: OrchestrationContext) -> list[str]:
        """生成执行步骤
        ...
        logger.debug(f"Generated {len(steps)} execution steps, request_id: {context.request_id}")
    ...
    async def _rollback_delete_data(self, action: dict[str, Any], context: OrchestrationContext) -> None:
        ...
        if plan_id:
            try:
                ...
                logger.info(f"Plan deleted during rollback: {plan_id}, request_id: {context.request_id}")
            except Exception as e:
                logger.error(f"Failed to delete plan during rollback: {plan_id}, error: {e!s}")
    ...
    async def _rollback_cleanup_resources(self, action: dict[str, Any], context: OrchestrationContext) -> None:
        ...
        for cache_key in cache_keys:
            try:
                ...
                logger.info(f"Cache cleared during rollback: {cache_key}, request_id: {context.request_id}")
            except Exception as e:
                logger.error(f"Failed to clear cache during rollback: {cache_key}, error: {e!s}")

    async def generate_plan(self, request: PlanGenerationRequest, request_id: str, user_id: str | None = None) -> PlanGenerationResponse:
        """生成投资方案
