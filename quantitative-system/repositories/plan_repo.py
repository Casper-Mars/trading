...
class PlanRepo:
    ...
    def get_by_id(self, plan_id: int) -> Optional[TradingPlan]:
        """根据ID获取交易方案
    ...
    def get_by_date(self, plan_date: date) -> Optional[TradingPlan]:
        """根据日期获取交易方案
        ...
        try:
            with self._get_session() as session:
                statement = select(TradingPlan).where(
    ...
    def get_today_plan(self) -> Optional[TradingPlan]:
        """获取今日交易方案
    ...
    def get_latest_plan(self) -> Optional[TradingPlan]:
        """获取最新的交易方案
        ...
        try:
            with self._get_session() as session:
                statement = select(TradingPlan).order_by(
                    TradingPlan.plan_date.desc(),
                    TradingPlan.created_at.desc()
                )
                plan = session.exec(statement).first()
                return plan
    ...
    def get_by_type(self, plan_type: PlanType) -> List[TradingPlan]:
        ...
        try:
            with self._get_session() as session:
                statement = select(TradingPlan).where(
                    TradingPlan.plan_type == plan_type
                ).order_by(TradingPlan.plan_date.desc())
                plans = session.exec(statement).all()
                return list(plans)
    ...
    def update(self, plan_id: int, update_data: TradingPlanUpdate) -> Optional[TradingPlan]:
        """更新交易方案
                    ...
                    if hasattr(plan, field) and value is not None:
                        ...
                        if field in ['analysis_data', 'config']:
                            if isinstance(value, dict):
                                setattr(plan, field, json.dumps(value, ensure_ascii=False, default=str))
                            else:
    ...
    def search_by_content(self, keyword: str, limit: int = 10) -> List[TradingPlan]:
        ...
        try:
            with self._get_session() as session:
                ...
                ).order_by(TradingPlan.plan_date.desc()).limit(limit)
