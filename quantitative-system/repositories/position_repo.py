...
class PositionRepo:
    ...
    def create(self, position_data: PositionCreate) -> Position:
        ...
        try:
            with self._get_session() as session:
                ...
                logger.info(f"创建持仓成功: {position.symbol}, 数量: {position.quantity}")
                return position
    ...
    def update(self, position_id: int, update_data: PositionUpdate) -> Optional[Position]:
        """更新持仓
        ...
        try:
            with self._get_session() as session:
                ...
                # 重新计算市值和未实现盈亏
                if update_data.quantity is not None or update_data.current_price is not None:
                    position.market_value = position.quantity * position.current_price
                    position.unrealized_pnl = (
                        position.quantity * (position.current_price - position.avg_price)
                    )
    ...
    def get_portfolio_summary(self) -> Dict[str, any]:
        ...
        try:
            with self._get_session() as session:
                ...
                # 计算总盈亏
                total_pnl = (result.total_unrealized_pnl or Decimal('0')) + (result.total_realized_pnl or Decimal('0'))
                
                return {
                    'total_positions': result.total_positions or 0,
                    'total_market_value': float(result.total_market_value or Decimal('0')),
                    'total_unrealized_pnl': float(result.total_unrealized_pnl or Decimal('0')),
                    'total_realized_pnl': float(result.total_realized_pnl or Decimal('0')),
                    'total_pnl': float(total_pnl),
                    'long_positions': {
                        'count': long_result.long_count or 0,
                        'market_value': float(long_result.long_market_value or Decimal('0'))
                    },
                    'short_positions': {
                        'count': short_result.short_count or 0,
                        'market_value': float(short_result.short_market_value or Decimal('0'))
                    }
                }
