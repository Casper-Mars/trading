...
class BacktestRepo:
    ...
    def create(self, backtest_data: BacktestCreate) -> BacktestResult:
        ...
        try:
            with self._get_session() as session:
                ...
                logger.info(f"创建回测任务成功: {backtest.strategy_name}, ID: {backtest.id}")
                return backtest
    ...
    def get_by_id(self, backtest_id: int) -> Optional[BacktestResult]:
        ...
        try:
            with self._get_session() as session:
                statement = select(BacktestResult).where(BacktestResult.id == backtest_id)
    ...
    def get_paginated(
        ...
        try:
            with self._get_session() as session:
                ...
                if strategy_name:
                    conditions.append(BacktestResult.strategy_name.contains(strategy_name))
    ...
    def update(self, backtest_id: int, update_data: BacktestUpdate) -> Optional[BacktestResult]:
        """更新回测结果
            ...
            with self._get_session() as session:
                ...
                for field, value in update_dict.items():
                    if hasattr(backtest, field) and value is not None:
                        # 特殊处理JSON字段
                        if field in ['strategy_params', 'backtest_config', 'metrics', 'raw_data']:
                            if isinstance(value, dict):
                                setattr(backtest, field, json.dumps(value, ensure_ascii=False, default=str))
                            else:
    ...
    def update_status(self, backtest_id: int, status: BacktestStatus, error_message: Optional[str] = None) -> bool:
        """更新回测状态
    ...
    def save_results(
        ...
        try:
            with self._get_session() as session:
                ...
                # 保存原始数据（如果提供）
                if raw_data:
                    backtest.raw_data = json.dumps(raw_data, ensure_ascii=False, default=str)
    ...
    def get_strategy_performance_summary(self, strategy_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"获取策略性能汇总失败: strategy_name={strategy_name}, 错误: {e}")
            return {
                'strategy_name': strategy_name,
                'total_backtests': 0,
                'avg_return': 0.0,
    ...
    def get_pending_backtests(self) -> List[BacktestResult]:
        ...
        try:
            with self._get_session() as session:
                statement = select(BacktestResult).where(
                    BacktestResult.status == BacktestStatus.PENDING
