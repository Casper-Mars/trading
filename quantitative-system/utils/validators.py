...
def validate_stock_code(stock_code: str) -> bool:
    ...
    if not re.match(pattern, stock_code.upper()):
        raise DataValidationError(f"股票代码格式错误: {stock_code}，应为6位数字.交易所代码")
...
def validate_positive_number(value: Union[int, float, Decimal], 
                           field_name: str = "数值") -> bool:
    """验证正数
...
def validate_percentage(value: Union[int, float, Decimal], 
                       field_name: str = "百分比") -> bool:
    """验证百分比（0-100）
