...
// Create 创建任务记录
func (d *dataTaskDAO) Create(ctx context.Context, task *model.DataTask) error {
...
// GetByID 根据ID获取任务
func (d *dataTaskDAO) GetByID(ctx context.Context, id uint64) (*model.DataTask, error) {
...
func (d *dataTaskDAO) GetByName(ctx context.Context, taskName string) (*model.DataTask, error) {
	var task model.DataTask
...
func (d *dataTaskDAO) GetByType(ctx context.Context, taskType string, limit, offset int) ([]*model.DataTask, error) {
...
func (d *dataTaskDAO) GetByStatus(ctx context.Context, status int8, limit, offset int) ([]*model.DataTask, error) {
	var tasks []*model.DataTask
...
func (d *dataTaskDAO) GetEnabledTasks(ctx context.Context) ([]*model.DataTask, error) {
	var tasks []*model.DataTask
	if err := d.db.WithContext(ctx).Where("status = ?", model.TaskStatusEnabled).Find(&tasks).Error; err != nil {
...
func (d *dataTaskDAO) GetTasksToRun(ctx context.Context) ([]*model.DataTask, error) {
	var tasks []*model.DataTask
	now := time.Now()
	if err := d.db.WithContext(ctx).
		Where("status = ? AND next_run_at IS NOT NULL AND next_run_at <= ?", model.TaskStatusEnabled, now).
...
// GetAll 获取所有任务
func (d *dataTaskDAO) GetAll(ctx context.Context, limit, offset int) ([]*model.DataTask, error) {
	var tasks []*model.DataTask
...
// Update 更新任务
func (d *dataTaskDAO) Update(ctx context.Context, task *model.DataTask) error {
...
func (d *dataTaskDAO) UpdateStatus(ctx context.Context, id uint64, status int8) error {
	if err := d.db.WithContext(ctx).Model(&model.DataTask{}).Where("id = ?", id).Update("status", status).Error; err != nil {
...
func (d *dataTaskDAO) UpdateRunStats(ctx context.Context, id uint64, lastRunAt, nextRunAt *time.Time, runCount, successCount, failureCount uint64) error {
	...
	if err := d.db.WithContext(ctx).Model(&model.DataTask{}).Where("id = ?", id).Updates(updates).Error; err != nil {
...
func (d *dataTaskDAO) Delete(ctx context.Context, id uint64) error {
	if err := d.db.WithContext(ctx).Delete(&model.DataTask{}, id).Error; err != nil {
...
func (d *dataTaskDAO) Count(ctx context.Context) (int64, error) {
	var count int64
	if err := d.db.WithContext(ctx).Model(&model.DataTask{}).Count(&count).Error; err != nil {
