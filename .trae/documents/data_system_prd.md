...
| 手动数据采集触发 | 支持手动触发全量/增量/指定范围数据采集 | P0  | 中   | 1周   |
...
**手动数据采集触发模块**
- 全量数据采集：支持手动触发全A股历史数据的完整采集
- 增量数据采集：支持手动触发指定日期的增量数据更新
- 指定股票采集：支持手动触发特定股票代码的数据采集
- 日期范围采集：支持手动触发指定时间范围内的数据采集
- 采集任务管理：提供任务状态查询和进度跟踪功能
...
- 手动数据采集：提供手动触发数据采集的API接口
- 采集任务管理：提供采集任务状态查询和管理接口
